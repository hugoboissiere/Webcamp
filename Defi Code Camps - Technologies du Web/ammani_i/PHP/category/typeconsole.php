...
<!-- 			<button type="button" id="sortname">Tri par nom</button>
			...
			<?php 
			$reponse = $bdd->query('SELECT Produits.Libelle, Produits.Image, Produits.Prix_vente, Produits.ID FROM Produits, Categories, Categorie_produit WHERE Produits.ID = Categorie_produit.ID_produit AND Categories.ID = Categorie_produit.ID_categorie AND Categories.ID = "' . $_GET['ID'] . '"');

			while ($donnees = $reponse->fetch())
			{
				// echo "<span class ='produitnom'>" . $donnees['Libelle'] . "</span><a href='". "../articles/game.php?ID=" . $donnees['ID'] . "'><img class='jaquette' src='" . $donnees['Image'] . "'></a>" . $donnees['Prix_vente'] . "€" . "<br>";
